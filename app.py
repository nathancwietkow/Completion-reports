...
PROJECT_DESCRIPTIONS = {
    "Full Asset Reline": "fully reline",
    "Cooling Tower Reline": "fully reline",
    "Cooling Tower Repair": "repair",
    "Bolt restoration and lining": "restore corroded bolt work and reline these areas",
    "Steel restorations and lining": "restore corroded steel work and reline these areas",
    "Asset clean and disinfections": "clean and disinfect",
    "Repairing cracks/patch repairs": "repair and reline specified damaged areas of",
    "Dividing wall repairs": "repair",
    "Tanking plant room floors": "repair and tank the plant room floor",
}
...
SCOPE_OF_WORKS = {
    ...
    "Asset clean and disinfections": "draining, drying and thoroughly cleaning. These will then be disinfected, ahead of refilling and reinstating back into the system.",
...
def generate_report():
    try:
        ...
        if company == "Key Environmental Services Ltd":
            company_contact_details = "01789 330830 | service@key-environmental.co.uk | www.watertankrelining.co.uk"
        ...
        # Generate main project description with singular/plural
        main_project_description = PROJECT_DESCRIPTIONS.get(major_project, major_project)

        # Handle singular vs plural for asset references
        is_plural = asset_quantity and asset_quantity not in ('1', '1.0')
        if is_plural:
            # Pluralize asset type if needed
            if asset_type:
                # Simple pluralization - add 's' (may need customization)
        ...
        # Determine location preposition (on vs in)
        location_preposition = "in"
        if asset_location.lower() in ('roof', 'rooftop', 'top floor'):
        ...
        replacements = {
            ...
            '[MAIN_PROJECT]': main_project_description,
        ...
        # STEP 1: Replace all text placeholders (preserves formatting)
        def replace_paragraph_text(paragraph, replacements):
            """Replace placeholders in paragraph while preserving run formatting."""
            # Get full text
            ...
            # Apply replacements EXCEPT [SCOPE_OF_WORKS]
            new_text = full_text
            ...
            # Replace text in-place across runs, preserving formatting
            # This is complex: we rebuild the run list to match new_text structure
            # but preserve the original run formatting properties
            if not paragraph.runs:
                paragraph.add_run(new_text)
                return

            # Simple approach: keep first run's formatting, redistribute text
            ...
            # Add new text back to first run with preserved properties
            new_run = paragraph.add_run(new_text)
            # Copy formatting from original first run
