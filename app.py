...
def generate_report():
    try:
        ...
        # STEP 2: Handle [SCOPE_OF_WORKS] - split by newlines and create paragraphs
        for paragraph in doc.paragraphs:
            if '[SCOPE_OF_WORKS]' in paragraph.text:
                # Split scope of works text by double newlines
                scope_lines = scope_of_works.split('\n\n')

                # Clear existing runs
                for run in paragraph.runs:
                    run.text = ''

                # Add first part to existing paragraph
                if scope_lines:
                    paragraph.runs[0].text = scope_lines[0] if paragraph.runs else ''
                    if not paragraph.runs:
                        paragraph.add_run(scope_lines[0])
                ...
                for line in scope_lines[1:]:
                    ...
                    new_para_xml = (
                        ...
                        '<w:r><w:t>' + line.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;') + '</w:t></w:r>'
                        '</w:p>'
                    )
                    new_para = parse_xml(new_para_xml)
